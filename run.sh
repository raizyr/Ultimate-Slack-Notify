...
if [ ! -n "$DEPLOY" ]; then
  WERCKER_STEP="BUILD $RESULT  wercker:<$WERCKER_BUILD_URL|${WERCKER_BUILD_ID:0:7}>"
else
  WERCKER_STEP="DEPLOY $RESULT  wercker:<$WERCKER_DEPLOY_URL|${WERCKER_DEPLOY_ID:0:7}>"
fi
...
GIT_COMMIT_URL="github:<https://$WERCKER_GIT_DOMAIN/$WERCKER_GIT_OWNER/$WERCKER_GIT_REPOSITORY/$GIT_TREE/$WERCKER_GIT_COMMIT|${WERCKER_GIT_COMMIT:0:7}>"
