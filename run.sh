...
if [ ! -n "$DEPLOY" ]; then
  WERCKER_STEP="BUILD #<$WERCKER_BUILD_URL|${WERCKER_BUILD_ID:0:7}>"
else
  WERCKER_STEP="DEPLOY #<$WERCKER_DEPLOY_URL|${WERCKER_DEPLOY_ID:0:7}>"
fi
...
if [ ! -n "$DEPLOY" ]; then
  SLACK_MESSAGE="$WERCKER_STEP ($GIT_COMMIT_URL) of $GIT_PROJECT by $WERCKER_STARTED_BY $RESULT $WERCKER_TIME_SPENT"
else
  SLACK_MESSAGE="$WERCKER_STEP ($GIT_COMMIT_URL) of $GIT_PROJECT to $WERCKER_DEPLOYTARGET_NAME by $WERCKER_STARTED_BY $RESULT $WERCKER_TIME_SPENT"
fi
