...
SLACK_FAILED_ICON_URL=$WERCKER_ULTIMATE_SLACK_NOTIFY_FAILED_ICON_URL
SLACK_PASSED_ICON_URL=$WERCKER_ULTIMATE_SLACK_NOTIFY_PASSED_ICON_URL
...
if [ ! -n "$DEPLOY" ]; then
  WERCKER_STEP="<$WERCKER_BUILD_URL|BUILD> $RESULT"
else
  WERCKER_STEP="<$WERCKER_DEPLOY_URL|DEPLOY> $RESULT"
fi

if [ "$WERCKER_RESULT" = "passed" ]; then
  SLACK_COLOR="good"
  if [ -n "$SLACK_PASSED_ICON_URL" ]; then
    SLACK_AVATAR="\"icon_url\":\"$SLACK_PASSED_ICON_URL\","
  fi
else
  SLACK_COLOR="danger"
  if [ -n "$SLACK_FAILED_ICON_URL" ]; then
    SLACK_AVATAR="\"icon_url\":\"$SLACK_FAILED_ICON_URL\","
  fi
fi
