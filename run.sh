...
if [ ! -n "$DEPLOY" ]; then
  WERCKER_STEP="<$WERCKER_BUILD_URL|BUILD:${WERCKER_BUILD_ID:0:7}> $RESULT"
else
  WERCKER_STEP="<$WERCKER_DEPLOY_URL|DEPLOY:${WERCKER_DEPLOY_ID:0:7}> $RESULT"
fi
...
GIT_COMMIT_URL="git:<https://$WERCKER_GIT_DOMAIN/$WERCKER_GIT_OWNER/$WERCKER_GIT_REPOSITORY/$GIT_TREE/$WERCKER_GIT_COMMIT|${WERCKER_GIT_COMMIT:0:7}>"
