...
export SLACK_ICON_URL=$WERCKER_ULTIMATE_SLACK_FAILED_ICON_URL
export SLACK_ICON_EMOJI=$WERCKER_ULTIMATE_SLACK_FAILED_EMOJI
...
export SLACK_WERCKER_TIME_SPENT=$(($(date +%s) - $WERCKER_MAIN_PIPELINE_STARTED)) | awk '{print int($1/60)" min "int($1%60)" sec"}'
...
if [ ! -n "$DEPLOY" ]; then
  export SLACK_MESSAGE="$SLACK_WERCKER_STEP ($SLACK_GIT_COMMIT_URL) of $SLACK_GIT_PROJECT by $WERCKER_STARTED_BY $WERCKER_RESULT in $SLACK_WERCKER_TIME_SPENT."
else
  export SLACK_MESSAGE="$SLACK_WERCKER_STEP ($SLACK_GIT_COMMIT_URL) of $SLACK_GIT_PROJECT to $WERCKER_DEPLOYTARGET_NAME by $WERCKER_STARTED_BY $WERCKER_RESULT in $SLACK_WERCKER_TIME_SPENT."
fi
