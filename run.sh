...
if [ ! -n "$DEPLOY" ]; then
  export WERCKER_STEP="Build #<$WERCKER_BUILD_URL|${WERCKER_BUILD_ID:0:5}>"
else
  export WERCKER_STEP="Deploy #<$WERCKER_DEPLOY_URL|${WERCKER_DEPLOY_ID:0:5}>"
fi
...
export GIT_COMMIT_URL="<https://$WERCKER_GIT_DOMAIN/$WERCKER_GIT_OWNER/$WERCKER_GIT_REPOSITORY/$GIT_TREE/$WERCKER_GIT_COMMIT|${WERCKER_GIT_COMMIT:0:7}>"
export GIT_PROJECT="$WERCKER_APPLICATION_OWNER_NAME/$WERCKER_APPLICATION_NAME@$WERCKER_GIT_BRANCH"

if [ ! -n "$DEPLOY" ]; then
  export SLACK_MESSAGE="$WERCKER_STEP ($GIT_COMMIT_URL) of $GIT_PROJECT by $WERCKER_STARTED_BY $WERCKER_RESULT."
else
  export SLACK_MESSAGE="$WERCKER_STEP ($GIT_COMMIT_URL) of $GIT_PROJECT to $WERCKER_DEPLOYTARGET_NAME by $WERCKER_STARTED_BY $WERCKER_RESULT."
fi
